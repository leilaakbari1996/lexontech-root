...
namespace Lexontech\Root\app\Traits\AuthenticationSystem;

use Illuminate\Support\Carbon;
use Lexontech\Root\app\Models\AuthenticationSystem\AuthTemporaryUser;
...
trait SMS
{
    ...
    {
        AuthTemporaryUser::DeleteByPhone($phone);
        $otp = AuthTemporaryUser::create([
            'PhoneNumber' => $phone,
            'Otp' => $otp
        ]);
    ...
    protected function VerifyOtp($data)
    {
        ...
        $result = AuthTemporaryUser::FindRow($data);
        if(empty($result))
        {
            return [
        ...
        AuthTemporaryUser::DeleteByPhone($data['PhoneNumber']);
        $now = Carbon::now();
        if($now > $expire)
        {//expired
            return [
